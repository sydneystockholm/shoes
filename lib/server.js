...
/**
 * Add a useragent alias.
 *
 * @param {String} name - e.g. '<=ie8'
 * @param {RegExp} regexp - e.g. /MSIE [5-8]/
 */

Server.prototype.addUseragent = function (name, regexp) {
    this.useragents[name] = regexp;
};
...
Server.prototype.redirectUseragent = function (name, redirect_to) {
    this.redirects[name] = redirect_to;
};
...
Server.prototype.useragentMiddleware = function () {
    ...
    return function (request, response, next) {
        ...
        //Determine useragent
        if (ua) {
            for (var key in self.useragents) {
                if (self.useragents[key].test(ua)) {
                    name = key;
                    break;
                }
            }
        }
        response.local('useragent', name);
        if (!name) {
            return next();
        }
        //Apply UA specific redirects
        if (name in self.redirects) {
            if (request.url !== self.redirects[name]) {
                return response.redirect(self.redirects[name]);
            }
        }
        next();
    };
