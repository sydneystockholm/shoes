...
Server.prototype.create = function () {
    ...
    app.use(function (request, response, next) {
        response.render = function (file, options) {
            file += '.html';
            try {
                var result = env.render(file, options || {});
                response.send(result);
            } catch (e) {
                log.error('Template %s: %s', file, e.message);
                response.end();
            }
        };
        next();
    ...
    app.use(function (request, response, next) {
        ...
        response.local('nonce', utils.md5(''+Date.now()).substr(0, 6));
        next();
    });

    //Setup the CSS / JS compiler + minifier
    express.compiler.compilers.less.compile = this.compileCss.bind(this);
    express.compiler.compilers.js = {
        match: /\.js$/
      , ext: '.js'
      , compile: this.compileJs.bind(this)
    };
...
Server.prototype.compileCss = function (str, callback) {
    var config = {
        compress : this.config.production
